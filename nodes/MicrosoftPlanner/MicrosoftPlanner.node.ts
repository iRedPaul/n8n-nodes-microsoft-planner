...
function getFormattedDateTimeOrThrow(
  ...
  const formattedValue = formatDateTime(trimmedValue);

  if (!formattedValue) {
    throw new NodeOperationError(
      context.getNode(),
      `Invalid ${fieldLabel}. Use an ISO 8601 value or another parseable date/time, for example "2026-03-08T14:30:00Z".`,
      { itemIndex }
    );
  }
...
function getMappedToolValueOrThrow<T extends string | number>(
  ...
  if (mappedValue === undefined) {
    const supportedValues = Object.keys(mapping)
      .filter((key) => Number.isNaN(Number(key)))
      .join(", ");

    throw new NodeOperationError(
      context.getNode(),
      `Invalid ${fieldLabel}. Supported values: ${supportedValues}.`,
      { itemIndex }
    );
  }
...
export class MicrosoftPlanner implements INodeType {
  description: INodeTypeDescription = {
    ...
    description:
      "Manage tasks, plans, buckets, and comments in Microsoft Planner",
    defaults: {
      ...
      try {
        if (resource === "task") {
          ...
          if (operation === "create") {
            ...
            const attachmentsUi =
              (additionalFields.attachmentsUi as IDataObject) ??
              getToolJsonCollection(
                additionalFields,
                "attachmentsJson",
                "attachments"
              );
            const checklistUi =
              (additionalFields.checklistUi as IDataObject) ??
              getToolJsonCollection(
                additionalFields,
                "checklistJson",
                "checklist"
            ...
            const toolPriority = parsePlannerPriority(
              additionalFields.priorityText
            );
            if (
              getTrimmedString(additionalFields.priorityText) &&
              toolPriority === undefined
            ) {
              throw new NodeOperationError(
                this.getNode(),
                "Invalid Priority Text field. Supported values: urgent, important, medium, low, 1, 3, 5, 9.",
                { itemIndex: i }
              );
            }
            if (toolPriority !== undefined) {
              body.priority = toolPriority;
            } else if (additionalFields.priority !== undefined) {
            ...
            if (additionalFields.assignments) {
              const emails = parseAssignments(
                additionalFields.assignments as string | string[]
              );
            ...
            if (additionalFields.description || attachmentsUi || checklistUi) {
              ...
              if (attachmentsUi && attachmentsUi.attachments) {
                ...
                if (mode === "manual" && attachments.items) {
                  ...
                  try {
                    ...
                    throw new NodeOperationError(
                      this.getNode(),
                      "Invalid JSON in Attachments (JSON) field. It must be an array of objects.",
                      { itemIndex: i }
                    );
                  }
              ...
              if (checklistUi && checklistUi.checklist) {
                ...
                if (mode === "manual" && checklist.items) {
                  ...
                  try {
                    checklistList =
                      typeof checklistJson === "string"
                        ? JSON.parse(checklistJson)
                        : checklistJson;
                  } catch (error) {
                    throw new NodeOperationError(
                      this.getNode(),
                      "Invalid JSON in Checklist (JSON) field. It must be an array of objects.",
                      { itemIndex: i }
                    );
                  }
          ...
          if (operation === "get") {
            ...
            const taskId =
              typeof taskIdParam === "string"
                ? taskIdParam
                : ((taskIdParam as IDataObject).value as string);
            const additionalFields = this.getNodeParameter(
              "additionalFields",
              i
            ) as IDataObject;
          ...
          if (operation === "getAll") {
            ...
            if (filterBy === "plan") {
              if (!getTrimmedString(planId)) {
                throw new NodeOperationError(
                  this.getNode(),
                  "Plan ID is required when listing tasks by plan.",
                  { itemIndex: i }
                );
              }
              endpoint = `/planner/plans/${planId}/tasks`;
              ...
              if (!getTrimmedString(bucketIdValue)) {
                throw new NodeOperationError(
                  this.getNode(),
                  "Bucket ID is required when listing tasks by bucket.",
                  { itemIndex: i }
                );
              }
              endpoint = `/planner/buckets/${bucketIdValue}/tasks`;
          ...
          if (operation === "update") {
            ...
            const attachmentsUi =
              (updateFields.attachmentsUi as IDataObject) ??
              getToolJsonCollection(
                updateFields,
                "attachmentsJson",
                "attachments"
            ...
            const checklistUi =
              (updateFields.checklistUi as IDataObject) ??
            ...
            const toolPriority = parsePlannerPriority(
              updateFields.priorityText
            );
            if (
              getTrimmedString(updateFields.priorityText) &&
              toolPriority === undefined
            ) {
              throw new NodeOperationError(
                this.getNode(),
            ...
            if (toolPriority !== undefined) {
              body.priority = toolPriority;
            } else if (updateFields.priority !== undefined) {
            ...
            if (updateFields.assignments) {
              const emails = parseAssignments(
                updateFields.assignments as string | string[]
              );
              const userIds: string[] = [];
            ...
            if (updateFields.description || attachmentsUi || checklistUi) {
              const details = await microsoftApiRequest.call(
                this,
                "GET",
                `/planner/tasks/${taskId}/details`
              ...
              const attachmentsForReplace =
                attachmentsUi?.attachments as IDataObject;
              const attachmentsOperationMode =
                attachmentsForReplace?.operationMode as string;
              if (
                attachmentsOperationMode === "replace" &&
                details.references
              ) {
              ...
              const checklistOperationMode =
                checklistForReplace?.operationMode as string;
            ...
            const files = Object.keys(references).map((encodedUrl) => {
              ...
              return {
                ...
                lastModifiedDateTime:
                  references[encodedUrl].lastModifiedDateTime,
        ...
        if (resource === "plan") {
          ...
          if (operation === "getAll") {
            ...
            if (scope === "my") {
              ...
              if (!getTrimmedString(groupId)) {
                throw new NodeOperationError(
                  this.getNode(),
                  "Group ID is required when Scope Text is group.",
                  { itemIndex: i }
                );
              }
              ...
              throw new NodeOperationError(
                this.getNode(),
                "Invalid scope for plan getAll operation",
                {
                  itemIndex: i,
                }
              );
          ...
          if (operation === "update") {
            ...
            if (updateFields.categoryDescriptions) {
              const categories =
                updateFields.categoryDescriptions as IDataObject;
            ...
            if (updateFields.sharedWithJson) {
              ...
              try {
                sharedWithParsed = JSON.parse(
                  updateFields.sharedWithJson as string
                ) as IDataObject;
              } catch (error) {
                throw new NodeOperationError(
                  this.getNode(),
                  "Invalid JSON in Shared With (plannerUserIds JSON) field",
                  {
                    itemIndex: i,
                  }
                );
        ...
        if (resource === "comment") {
          ...
          if (operation === "create") {
            ...
            const toolContentType = getMappedToolValueOrThrow(
              this,
              additionalFields.contentTypeText,
              {
                text: "text",
                html: "html",
              },
            ...
            const contentType =
              toolContentType ??
              ((additionalFields.contentType as string) || "text");
